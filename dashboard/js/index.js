...
SiriWave.prototype = {
  ...
  _drawLine: function (attenuation, color, width) {
    ...
    // 	this.ctx.lineTo(x, y);
    // }
    for (var i = -this.K; i <= 0; i += 0.01) {
      x = this.width * ((i + this.K) / (this.K));
      y = this.height / 2 + this.noise * this._globalAttenuationFn(i) * (1 / attenuation) * Math.sin(this.F * i - this.phase);
      this.ctx.lineTo(x, y);
    }
    this.ctx.stroke();
  ...
  _draw: function () {
    ...
    this._drawLine(1, 'rgba(72,85,191,1)', 3);
    this._drawLine(1, 'rgba(72,85,191,.3)', 8);
    this._drawLine(2, 'rgba(47,59,129,0.8)', 2);
    this._drawLine(2, 'rgba(47,59,129,0.3)', 8);
